...
function getChartLabelArray(r) {
    r = $(r).attr("data-labels");
    return (r = JSON.parse(r)).map(function (r) {
        r = r.replace(" ", "");
        if (-1 == r.indexOf("--")) return r;
        r = getComputedStyle(document.documentElement).getPropertyValue(r);
...
function getChartValuesArray(r) {
    r = $(r).attr("data-values");
    return (r = JSON.parse(r)).map(function (r) {
        r = r.replace(" ", "");
         
        if (-1 == r.indexOf("--")) return parseInt(r);
        r = getComputedStyle(document.documentElement).getPropertyValue(r);
        return r || void 0;
    });
}
...
var minichart1Colors = getChartColorsArray("#mini-chart1"),
    options = {
        series: [{ data: [5, 10, 15, 22, 36, 15, 33, 42, 59, 49, 32, 12, 65, 92, 95] }],
...
var piechartColors = getChartColorsArray("#wallet-balance"),
    pieLabels = getChartLabelArray("#wallet-balance"),
    pieValues = getChartValuesArray("#wallet-balance"),
    options = {
        series: pieValues,
