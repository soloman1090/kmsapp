...
class ActivatePortfolios extends Controller
{
    public function index()
    {
        ...
        $investments = User::join('user_investments', 'user_investments.user_id', '=', 'users.id')
            ...
            ->get([
                'users.name as username', 'users.email', 'users.id as user_id', 'investment_packages.name as packagename',
        ...
        for ($i = 0; $i < count($investments); $i++) {
            ...
            if ($investments[$i]['txn_id'] != "TRX") {
                ...
                if ($d1 < $d2) {
                    ...
                    $data = [
                        'days' => $totalDaysDiff, 'daysLeft' => $dayLeft, 'username' => $investments[$i]['username'], 'user_id' => $investments[$i]['user_id'], 'email' => $investments[$i]['email'],
                ...
                if ($investments[$i]['status'] == "pending") {
                    ...
                    if ($txn_id != "wallet" && $txn_id != null  && $thisInvestment->currency != null) {
                        ...
                        //CHECK IF ITS THROUGH WALLET
                    } else if ($txn_id == "wallet") {
                        $investments[$i]['status'] = "completed";
                        $thisInvestment->status = "completed";
                        $thisInvestment->active = true;
                        $thisInvestment->duration = $monthCount;
                    ...
                    if ($transactionCompleted == true) {
                        ...
                        $activity = new Activities;
                        $activity->title = "Portfolio Initialized";
                        $activity->user_id = $investments[$i]['user_id'];
                        $activity->user_investments_id = $investments[$i]['investment_id'];
                        $activity->investment_packages_id = $investments[$i]['package_id'];
                ...
                Please Login to view investments",
                        ...
                        if ($user->referred_by != null && $user->referred_by != "") {
                            ...
                            if ($referredUser != null) {
                                ...
                                //Get Wallet Balance
                                $currentAmount = 0;
                                if ($referredUser->referral_wallet == null) {
                                    $currentAmount = 0;
                                } else {
                                    $currentAmount = $referredUser->referral_wallet;
                                }

                                //Save Wallet
                                $userInfo = UserInfo::where('user_id', $user->referred_by)->firstOrFail();
                                $userInfo->referral_wallet = $currentAmount + $bonusAmount;
                                $userInfo->update();
                                ...
                                if ($referredUser->referred_by != null) {
                                    ...
                                    if ($referredUser_stage2 != null) {
                                        ...
                                        //Get Wallet Balance
                                        $currentAmount = 0;
                                        if ($referredUser_stage2->referral_wallet == null) {
                                            $currentAmount = 0;
                                        } else {
                                            $currentAmount = $referredUser_stage2->referral_wallet;
                                        }
                                        $newAmount = $currentAmount + $bonusAmount_stage2;
                                        //Save Wallet
                                        $userInfo_stage2 = UserInfo::where('user_id', $referredUser_stage2->user_id)->firstOrFail();
                                        $userInfo_stage2->referral_wallet = $newAmount;
                                        ...
                                        if ($referredUser_stage2->referred_by != null) {
                                            ...
                                            if ($referredUser_stage3 != null) {
                                                ...
                                                //Get Wallet Balance
                                                $currentAmount_3 = 0;
                                                if ($referredUser_stage3->referral_wallet == null) {
                                                    $currentAmount_3 = 0;
                                                } else {
                                                    $currentAmount_3 = $referredUser_stage3->referral_wallet;
                                                }
                                                ...
                                                $userInfo_stage3->referral_wallet = $newAmount;
